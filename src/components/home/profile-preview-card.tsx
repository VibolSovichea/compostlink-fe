      ...
      <Card.Body className="flex flex-col rounded-xl gap-6 py-base">
        <div className="flex">
          <div className="flex flex-1 flex-col gap-2 ">
            <div className="text-x font-semibold text-secondary capitalize">Main balance</div>
            <div className="text-yellow-300 font-bold text-2xl">{`${points} pts`}</div>
            {variant === "user" ? (
              <div className="text-xs font-semibold text-balck ">USE THIS TO GET REWARDS</div>
            ) : (
              <div className="text-xs text-black">rewards points to our generators</div>
            )}
          </div>
          <div className="relative w-24 h-24 flex items-center justify-center">
            <CircularProgress size="100px" thickness="6px" value={points} color="yellow" trackColor="white" />
